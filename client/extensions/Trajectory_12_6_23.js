...
const polygon = new Polygon2D(0, 0, 1, 1, 2, 0, 1, -1);
console.log(polygon.isInside(1.5, 0.5)); 
console.log(polygon.isInside(0, 2));     
...
class HistoricData {
  ...
  async get_emissions() {
    if (this.dataLoaded === false) {// check if we already have the data
      await new Promise(r => setTimeout(r, 2000));
      this.historicData = [[1, 2, 3], [1, 2, 3], [1, 2, 3]]; //await response.json();
      const result = await fetch ('http://127.0.0.1:8001/NOxEmissionsfromMongo.js')
      this.historicData = await result.json()
      this.timestampHistory = this.historicData[0];
      this.NOxs = this.historicData[1];
  ...
  //       }, 1000);
  //       };
      //   const htmlResponse = await result2.text();
      //   const regex = /\[(.+)\]/; // Regex to extract the values between square brackets
      //   const matches = regex.exec(htmlResponse);
  ...
  //     xhr.open('GET', 'http://127.0.0.1:8001/DatafromMongoLive.js');
  //     xhr.onload = () => {
  //       if (xhr.status === 200) {
  //         const data = JSON.parse(xhr.responseText);
  //         this.historicData=data.historicData;
  //         this.realData=data.realData;
  //         console.log(this.realData)
  //         this.dataLoaded = true;
  //         this.realDataLoaded = true;
  //         this.timestampHistory=data.timestampHistory;
  //         resolve(this.realData);
  //       } else {
  ...
  //   return this.historicData[0].filter(point => {
  //   // console.log(this.historicData)
  //   const filteredTimestamps = this.timestampHistory.filter(point => {
  //     const pointTime = new Date(point).getTime();
  //     return !isNaN(pointTime) && pointTime >= startTime && pointTime <= endTime;
  //   });
  //   const filteredLons = this.lons.filter((lon, index) => filteredTimestamps.includes(this.timestampHistory[index]));
  //   const filteredLats = this.lats.filter((lat, index) => filteredTimestamps.includes(this.timestampHistory[index]));
...
export class Trajectory extends Autodesk.Viewing.UI.DockingPanel {
  ...
  initialize() {
    ...
    Heatmap.addEventListener('click', async event => {
      ...
      // Add your heatmap creation logic here
    });
  ...
  async loadHeatmap() {
    ...
    this.polygon2 = new Polygon2D(77.01471138,107.1271324,77.01471138+15,107.1271324+15,77.01471138+15,107.1271324-15,77.01471138-15,107.1271324-15,77.01471138-15,107.1271324+15)
    ...
    polygonCollection.push(this.polygon1,this.polygon2,this.polygon3,this.polygon4,this.polygon5,this.polygon6,this.polygon7,this.polygon8,this.polygon9,this.polygon10,this.polygon11,this.polygon12,this.polygon13,this.polygon14,this.polygon15,this.polygon16,this.polygon17,this.polygon18)  
    ...
    for (let polygon of polygonCollection) {
      ...
      for (let i = 0; i < positiondata[0].length; i++) {
        ...
        if (polygon.isInside(lmv.x, lmv.y)) {
          polygon.weight += 1;
        }
      }
    ...
    console.log(this.polygon2.isInside(75, 112.1271324));
    const testValues = {
        "Earth-sensor-1": {
            "currentIndex": 0,
            "NOx": [
